...
function delayUntil(hour, minute, extraDays = 0) {
  ...
  if (days === 0 && hour * 60 + minute <= now.hour * 60 + now.minute) {
    days = 1;
  }
...
function parseReminder(line) {
  ...
  // The time portion is greedy so a clock such as 1:14 PM is kept intact.
  // The separator is specifically the colon immediately before the quoted message.
  const tomorrow = text.match(/^remind\s+me\s+tomorrow(?:\s+at\s+(.+))?\s*:\s*["“](.*)["”]\s*$/i);
  if (tomorrow) {
  ...
  const recurring = text.match(/^remind\s+me\s+every\s+day\s+at\s+(.+)\s*:\s*["“](.*)["”]\s*$/i);
  if (recurring) {
  ...
  const absolute = text.match(/^remind\s+me\s+(?:at\s+)?(.+)\s*:\s*["“](.*)["”]\s*$/i);
  if (absolute) {
...
if (typeof originalOnLine === "function") {
  readline.Interface.prototype._onLine = function patchedOnLine(line) {
    ...
    if (permission) {
      void (async () => {
        try {
          const action = permission[1].toLowerCase();
          const capability = permission[2];
